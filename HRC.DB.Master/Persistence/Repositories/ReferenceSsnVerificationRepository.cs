...
{
    /// <summary>
    /// Extends the <see cref="Repository{TEntity}"/> for <see cref="ReferenceSsnVerification"/>, implementing
    /// <see cref="IReferenceSsnVerificationRepository"/>, using the <see cref="MasterDbContext"/> to get,
    /// find with predicate, add, or remove <see cref="ReferenceSsnVerification"/> objects in the database in
    ...
    /// </remarks>
    public class ReferenceSsnVerificationRepository : Repository<ReferenceSsnVerification>, IReferenceSsnVerificationRepository
    {
        public ReferenceSsnVerificationRepository(DbContext context)
            : base(context)
        {
        }
    }
